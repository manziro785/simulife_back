import { Router, Request, Response } from "express";
import { Character } from "../models/Character";
import { DayStory } from "../models/DayStoty";
import { charactersData } from "../data/characters";

const router = Router();

router.post("/characters", async (_req: Request, res: Response) => {
  try {
    const created = await Promise.all(
      charactersData.map((char) =>
        Character.create({
          id: char.id,
          name: char.name,
          description: char.description,
          avatar: char.avatar,
          money: char.stats.money,
          energy: char.stats.energy,
          mood: char.stats.mood,
        })
      )
    );
    res
      .status(201)
      .json({ message: "Персонажи добавлены", count: created.length });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Ошибка при добавлении персонажей" });
  }
});

router.post("/days", async (_req: Request, res: Response) => {
  try {
    const daysData = [
      {
        day: 1,
        title: "Первая зарплата",
        description: "Сегодня ты получил свою первую зарплату за подработку.",
        context: "У тебя в руках конверт с деньгами. Что ты сделаешь?",
        choices: [
          {
            id: "invest",
            text: "Отложить 50% на будущее",
            impact: { money: 150, energy: 0, mood: -10 },
          },
          {
            id: "spend",
            text: "Потратить на то, что хочется",
            impact: { money: -100, energy: 0, mood: 30 },
          },
          {
            id: "balance",
            text: "Отложить 20% и немного потратить",
            impact: { money: 50, energy: 0, mood: 10 },
          },
        ],
        lesson: {
          title: "Урок о деньгах",
          text: "Финансовая грамотность начинается с первого заработка. Откладывать деньги — это не ограничение, а инвестиция в будущее.",
          icon: "💰",
        },
      },
      {
        day: 2,
        title: "Экзамен близко",
        description: "Завтра важный экзамен, но друзья зовут на вечеринку.",
        context: "Твоя энергия на пределе. Как поступишь?",
        choices: [
          {
            id: "study",
            text: "Отказаться и готовиться к экзамену",
            impact: { money: 0, energy: -30, mood: -20 },
          },
          {
            id: "party",
            text: "Пойти на вечеринку и расслабиться",
            impact: { money: -50, energy: -40, mood: 40 },
          },
          {
            id: "quick-study",
            text: "Быстро повторить материал и лечь спать",
            impact: { money: 0, energy: 20, mood: 10 },
          },
        ],
        lesson: {
          title: "Урок о приоритетах",
          text: "Баланс между учебой и отдыхом важен, но иногда нужно жертвовать краткосрочным удовольствием ради долгосрочных целей.",
          icon: "📚",
        },
      },
      {
        day: 3,
        title: "Предложение подработки",
        description: "Тебе предложили подработку на выходных.",
        context: "Это хорошие деньги, но ты уже устал за неделю.",
        choices: [
          {
            id: "accept",
            text: "Принять — деньги нужны",
            impact: { money: 200, energy: -50, mood: -15 },
          },
          {
            id: "decline",
            text: "Отказаться и отдохнуть",
            impact: { money: 0, energy: 40, mood: 25 },
          },
          {
            id: "half-day",
            text: "Договориться на полдня",
            impact: { money: 100, energy: -20, mood: 5 },
          },
        ],
        lesson: {
          title: "Урок об энергии",
          text: 'Твоя энергия — ограниченный ресурс. Постоянная работа без отдыха приводит к выгоранию. Умей говорить "нет".',
          icon: "⚡",
        },
      },
      {
        day: 4,
        title: "Спонтанная покупка",
        description: "Ты увидел в магазине вещь, о которой давно мечтал.",
        context: "Цена высокая, но это то, что ты хотел. Купить?",
        choices: [
          {
            id: "buy",
            text: "Купить — жизнь одна",
            impact: { money: -150, energy: 0, mood: 30 },
          },
          {
            id: "wait",
            text: "Подождать и накопить",
            impact: { money: 0, energy: 0, mood: -10 },
          },
          {
            id: "research",
            text: "Изучить альтернативы",
            impact: { money: 0, energy: -10, mood: 5 },
          },
        ],
        lesson: {
          title: "Урок об импульсивности",
          text: "Спонтанные покупки приносят краткосрочную радость, но могут подорвать финансовую стабильность. Планирование — твой союзник.",
          icon: "🛍️",
        },
      },
      {
        day: 5,
        title: "Конфликт с другом",
        description: "Ты поссорился с другом из-за недоразумения.",
        context: "Это влияет на твое настроение. Что делать?",
        choices: [
          {
            id: "apologize",
            text: "Извиниться первым",
            impact: { money: 0, energy: -10, mood: 20 },
          },
          {
            id: "ignore",
            text: "Игнорировать ситуацию",
            impact: { money: 0, energy: 0, mood: -30 },
          },
          {
            id: "talk",
            text: "Спокойно обсудить проблему",
            impact: { money: 0, energy: -15, mood: 30 },
          },
        ],
        lesson: {
          title: "Урок о взаимоотношениях",
          text: "Здоровые отношения строятся на общении. Умение признавать ошибки и говорить о проблемах — признак зрелости.",
          icon: "🤝",
        },
      },
      {
        day: 6,
        title: "Новое увлечение",
        description: "Ты нашел новое хобби, которое тебя вдохновляет.",
        context: "Но оно требует времени и денег. Стоит ли развивать?",
        choices: [
          {
            id: "pursue",
            text: "Вложиться полностью",
            impact: { money: -100, energy: -20, mood: 40 },
          },
          {
            id: "skip",
            text: "Отказаться — сейчас не время",
            impact: { money: 0, energy: 0, mood: -20 },
          },
          {
            id: "moderate",
            text: "Заниматься понемногу",
            impact: { money: -30, energy: -10, mood: 20 },
          },
        ],
        lesson: {
          title: "Урок о саморазвитии",
          text: "Увлечения делают жизнь насыщенной. Инвестиции в себя и свои интересы — это инвестиции в счастье.",
          icon: "🎨",
        },
      },
      {
        day: 7,
        title: "Конец недели",
        description:
          "Подводя итоги недели, ты понимаешь, как много изменилось.",
        context: "Ты стал мудрее. Что ты выберешь на будущее?",
        choices: [
          {
            id: "reflect",
            text: "Проанализировать свои решения",
            impact: { money: 0, energy: -10, mood: 25 },
          },
          {
            id: "celebrate",
            text: "Отпраздновать достижения",
            impact: { money: -80, energy: -20, mood: 35 },
          },
          {
            id: "plan",
            text: "Составить план на следующую неделю",
            impact: { money: 0, energy: -15, mood: 20 },
          },
        ],
        lesson: {
          title: "Урок о рефлексии",
          text: "Каждое решение — это урок. Умение анализировать свои действия помогает расти и не повторять ошибок.",
          icon: "🌟",
        },
      },
    ];

    const created = await Promise.all(
      daysData.map((day) =>
        DayStory.create({
          day: day.day,
          title: day.title,
          description: day.description,
          context: day.context,
          choices: day.choices,
          lesson: day.lesson,
        })
      )
    );
    res.status(201).json({ message: "Дни добавлены", count: created.length });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Ошибка при добавлении дней" });
  }
});

export default router;
